...
void LCD_INIT()
{
	...
	LCD_sendCommand(LCD_FUNCTION_4BIT_2LINES); /* use 2-line lcd + 4-bit Data Mode + 5*7 dot display Mode */
#endif
...
void LCD_displayCharacter(unsigned char character)
{
	...
	_delay_ms(1); /* delay for processing Th = 13ns */
#elif (DATA_BITS_MODE == 8)
	LCD_DATA_PORT = character; /* out the required command to the data bus D0 --> D7 */
	_delay_ms(1); /* delay for processing Tdsw = 100ns */
	CLEAR_BIT(LCD_CTRL_PORT,EN); /* disable LCD E=0 */
	_delay_ms(1); /* delay for processing Th = 13ns */
#endif
